...
export const makeSocket = (config) => {
    ...
    const uploadPreKeys = async (count = MIN_PREKEY_COUNT, retryCount = 0, force = false) => {
        ...
        const uploadLogic = async (retryCountAtual) => {
            logger.info({ count, retryCount: retryCountAtual }, 'uploading pre-keys');
            ...
            try {
                ...
                if (retryCountAtual < 3) {
                    const backoffDelay = Math.min(1000 * Math.pow(2, retryCountAtual), 10000);
                    logger.info(`Retrying pre-key upload in ${backoffDelay}ms`);
                    await new Promise(resolve => setTimeout(resolve, backoffDelay));
                    // Recursiona na própria uploadLogic, NUNCA de volta em uploadPreKeys —
                    // uploadPreKeys ainda está com uploadPreKeysPromise pendente (é
                    // exatamente essa promise que essa função está resolvendo), então
                    // chamar uploadPreKeys(...) de novo aqui entrava no "if
                    // (uploadPreKeysPromise) await uploadPreKeysPromise" e ficava
                    // esperando a própria promise que só terminaria quando ELA
                    // terminasse — deadlock só quebrado pelo timeout de
                    // UPLOAD_TIMEOUT, reportando falha mesmo quando o retry teria
                    // dado certo rapidinho.
                    return uploadLogic(retryCountAtual + 1);
                }
                throw uploadError;
            }
        };
    ...
    const end = (error) => {
        ...
        // Limpa o repositório de sessão do Signal Protocol ao fechar — sem
        // isso, estado/listeners internos da sessão anterior podem continuar
        // vivos entre reconexões, contribuindo pra degradação de sessão.
        try {
        ...
        // Destrói o event buffer interno — cada reconexão cria um `ev` novo
        // (makeEventBuffer), então sem isso o antigo (com seus timers e
        // listeners internos) fica vivo indefinidamente em memória.
        try {
            ev.destroy();
        }
        catch { }
